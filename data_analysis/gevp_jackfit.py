...
T = c2pt11.shape[1]
T_hlf = T//2
#清空    output_file = "./output/fit_fwrd_output"中的所有信息

if os.path.exists("./output/fit_jack_output"):
    os.remove("./output/fit_jack_output")
...
for k in range(len(meson_list)):
    ...
    fit_colors = ['darkred', 'darkorange', 'darkblue', 'darkgreen', 'darkviolet']  # 拟合线条专用颜色
    markers = ['o', 's', 'D', '^', 'x']
    # 设置最大时间点用于绘图
    t_max_plot = 30
    ...
    for idx in range(chdim):
        try:
            # 调用自动拟合函数
            E0, errorbar, Tstart, Tend = fit_fwrd(k, idx, mode='jack')
            # 添加水平线
            ax.hlines(E0, Tstart, Tend, colors=fit_colors[idx], alpha=0.7, linestyle='-')
            # 添加误差带（半透明）
            ax.fill_between([Tstart, Tend],E0 - errorbar,E0 + errorbar,color=fit_colors[idx],alpha=0.2)
        except Exception as e:
            print(f"Error in auto_fit_fwrd for channel {idx}: {e}")
    ...
    ax.set_xlim(0, 31)
    # 保存图像
    fig.savefig("./output/%02d_lam_jack.png"%(k))