...
def readin_peram(peram_dir, conf_id, Nt, Nev, Nev1, t_source):
    ...
    for d_source in range(1, 4):
        f = open("%s/perams.%s.%i.%i" %
                 (peram_dir, conf_id, d_source, t_source), 'rb')
    ...
    # t_sink, ev_sink, ev_souce, d_sink, d_source, complex
    peram = peram.transpose(1, 4, 2, 3, 0, 5)
...
def readin_VVV_all(VVV_dir, Nev, Nev1, Nt, conf_id, Px, Py, Pz):
    ...
    for t in range(0, Nt):
        print(t)
        f = open("%s/VVV.t%03i.Px%iPy%iPz%i.conf%s" %
                 (VVV_dir, t, Px, Py, Pz, conf_id), 'rb')
        temp = np.fromfile(f, dtype='f8')
        temp_size = temp.size
...
def readin_VdV_all(VdV_dir, Nev, Nev1, Nt, conf_id, Px, Py, Pz):
    f = open("%s/%s/VdaggerV/VdaggerV.Px%dPy%dPz%d.conf%s" %
             (VdV_dir, conf_id, Px, Py, Pz, conf_id), 'rb')
...
#    L = int(T/2)
    ...
    if complex:
        ...
        savetxt(filename, _fdata, header=head,
                comments='', fmt=["%i", "%.32f", "%.32f"])
    else:
