# Now the program should determine the "Nev" parameter itself
...
def readin_eigvecs_device(eig_dir, t, Nev1, conf_id, Nx):
  ...
  eigvecs_torch=eigvecs_torch.cuda()
  return eigvecs_torch
...
def readin_peram_device(peram_dir, conf_id, Nt, Nev1, t_source):
    ...
    peram_torch=torch.from_numpy(peram)
    peram_torch=peram_torch.cuda()
    return peram_torch

# ------------------------------------------------------------------------------------------------
# read-in perambulator from all timeslices, and saved it in device memory
def readin_peram_all_cpu(peram_dir, conf_id, Nt, Nev1): # One less parameter compared to pervious verison(t_source)
    ...
    peram_cpu_all_torch=torch.from_numpy(peram_cpu_all)
    return peram_cpu_all_torch

# ------------------------------------------------------------------------------------------------
def readin_VVV_device(VVV_dir, t_source, Nev1,conf_id):
  ...
  VVV_torch=torch.from_numpy(VVV)
  VVV_torch=VVV_torch.cuda()
  return VVV_torch
...
def readin_VdV_all_device(VdV_dir, Nev1, Nt, conf_id, Px, Py, Pz):
  ...
  VdV_torch=torch.from_numpy(VdV)
  VdV_torch=VdV_torch.cuda()
  return VdV_torch	
